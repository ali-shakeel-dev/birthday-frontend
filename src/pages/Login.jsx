      ...
      <div className="bg-white rounded-xl shadow-md w-full max-w-sm p-8 border border-pink-100">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <input type="password" value={password} onChange={e => setPassword(e.target.value)} className="w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring focus:ring-pink-200 bg-pink-50" />
          </div>
          {error && <div className="text-sm text-red-500 text-center">{error}</div>}
          <button type="submit" className="w-full bg-pink-500 hover:bg-pink-600 text-white font-semibold rounded-lg py-2 transition">Login</button>
        </form>
        <div className="mt-6 text-center text-sm">
          No account? <Link to="/signup" className="text-pink-500 hover:underline">Sign up</Link>
        </div>
      </div>
    </div>
  );
